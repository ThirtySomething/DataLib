...
#include "testinc.h"
// *****************************************************************************
void handleBlock_ui16_range(size_t index, bool flag, size_t start, size_t end, size_t offset)
{
    bool success = false;
    uint16_t valueWrite = UINT16_MAX;
    uint16_t valueRead = 0;

    for (size_t loop = start; loop < end; loop++)
    {
        success = storageWrite_ui16(index, loop + offset, valueWrite);
        TEST_CHECK(flag == success);
        TEST_MSG("storageWrite_ui16(%x, %x) failed", index, loop + offset);

        success = storageRead_ui16(index, loop + offset, &valueRead);
        TEST_CHECK(flag == success);
        TEST_MSG("storageRead_ui16(%x, %x) failed", index, loop + offset);
        if (flag)
        {
...
// *****************************************************************************
void handleBlock_ui16(size_t index, bool left, bool right, size_t offset, size_t limit)
{
    handleBlock_ui16_range(index, left, 0, (limit / 2), offset);
    handleBlock_ui16_range(index, right, (limit / 2), limit, offset);
}
// *****************************************************************************
void t_storage_ui16(void)
{
    Endianness eArr[] = {E_ENDIANNESS_BIG, E_ENDIANNESS_LITTLE};
    // printf("size of uint16_t is [%zu] bytes\t\t", sizeof(uint16_t));
    ...
    for (size_t endianness = 0; endianness < 2; endianness++)
    {
        ...
        if (success)
        {
            ...
            handleBlock_ui16(0, false, true, 0, PT_BYTE8);
            // Normal block, both halfs allowed
            handleBlock_ui16(1, true, true, 0, PT_BYTE8);
            // Last block, first half allowed
            handleBlock_ui16(2, true, false, 0, PT_BYTE8);
            // Invalid block, no half allowed
            handleBlock_ui16(3, false, false, 0, PT_BYTE8);
            // Valid block, invalid position, no half allowed
            handleBlock_ui16(1, false, false, PT_BYTE8, PT_BYTE8);
        }
